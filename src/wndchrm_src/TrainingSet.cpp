...
*/
    ...
    for (sig_index = 0; sig_index < n_sigs; sig_index++) {
        ...
        if (strcmp(featureset->ROIPath,"")){
            ...
            {
                printf ("LabeledImagePath doesn't exist!");
                return -1;
            }

            if(!boost::filesystem::is_regular_file(LabeledImagePath))
            {
                printf ("Corrupted File for LabeledImagePath");
                return -1;
            }
            ...
            for (int i = 0; i < imageLength; ++i) {
                for (int j = 0; j < imageWidth; ++j) {
                    uniqueClasses.push_back(LabeledImageMatrix[i][j]);
                }
            }
