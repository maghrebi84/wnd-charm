...
In addition to the above changes, a comprehensive list of Morphological algorithms was implemented in WND-CHARM at src/MorphologicalAlgorithms.cpp and can be invoked as an input argument using "--FeatureAlgorithmName Morphological". The implemented Morphological algorithms compute a total of 89 features that are available in the following three references.

1-Feature Extraction Plugin for WIPP from https://github.com/Priyaaxle/polus-plugins/tree/master/polus-feature-extraction-plugin.
2-MATLAB regionprops module from https://www.mathworks.com/help/images/ref/regionprops.html.
3-imea 2D shape features from https://git.rwth-aachen.de/ants/sensorlab/imea/-/tree/master.

It should be noted that for the common parameters with MATLAB's regionprops module, the computed values were consistently converted to produce the same results as MATLAB. WND-CHARM outputs the Morphological features in the order listed below which is in accordance to the order of the outputs from the above three reference. 
